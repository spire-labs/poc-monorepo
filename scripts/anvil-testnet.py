...
# Since GCE VMs are externally managed, we need to install deps like web3py using a venv
# The following methods help the script determine if a venv is already active, and create one if needed
...
def create_and_activate_venv(venv_path):
    """Create and activate a new virtual environment, then install dependencies."""
    ...
    subprocess.check_call([sys.executable, '-m', 'pip', 'install', 'web3 flask-cors'])
...
def main():
    ...
    # venv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'poc-venv')
